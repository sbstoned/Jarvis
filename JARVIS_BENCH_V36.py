"""Jarvis-Bench V36: repeatable end-to-end project-generation benchmark.

Examples:
  python JARVIS_BENCH_V36.py --list
  python JARVIS_BENCH_V36.py --limit 1
  python JARVIS_BENCH_V36.py --ids python_cli,stockpilot

Each selected case runs the real local Qwen pipeline and records elapsed time, result status,
returned ZIP, and final message in generated_projects/JARVIS_BENCH_V36_RESULTS.json.
"""
...
def main():
    ...
    out_path=jarvis.GENERATED_DIR/'JARVIS_BENCH_V36_RESULTS.json'
    results=[]
    for idx,(cid,prompt) in enumerate(selected,1):
        ...
        results.append(result);out_path.write_text(json.dumps({'version':'V36.2.0','results':results},indent=2),encoding='utf-8')
        print(json.dumps(result,indent=2),flush=True)
    passed=sum(1 for x in results if x.get('ok'))
    print(f'\nJarvis-Bench V36: {passed}/{len(results)} generation runs reached Jarvis COMPLETE.')
