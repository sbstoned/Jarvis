...
check('missing planned dependency detected',any('missing planned provider' in x for x in issues),issues)
...
check('status reports V36.2 planner',status.get('version')=='V36.2.0' and status.get('planner_mode')=='chunked-hierarchical-v36.2',status)

failed=[x for x in checks if not x[1]]
for name,ok,detail in checks:
    print(('PASS' if ok else 'FAIL')+' - '+name+(f' ({detail})' if detail and not ok else ''))
print(f'\nV36.2 planner regression: {len(checks)-len(failed)}/{len(checks)} passed')
