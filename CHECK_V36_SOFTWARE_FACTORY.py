...
check('V36 identity is current V36.2',getattr(M,'V36_VERSION','')=='36.2.0',getattr(M,'V36_VERSION',''))
...
with tempfile.TemporaryDirectory(prefix='v36_status_') as td:
    ...
    check('status reports V36.2',data.get('version')=='V36.2.0',data)
    check('status reports software factory engine',data.get('engine')=='UNIVERSAL_SOFTWARE_FACTORY',data)
...
print(f'\nV36 self-test: {len(checks)-len(failed)}/{len(checks)} passed')
