...
check('live status reports V37',data.get('version')=='V37.0.0' and data.get('real_run_hardening')=='V37',data)

failed=[x for x in checks if not x[1]]
for name,ok,detail in checks:
    print(('PASS' if ok else 'FAIL')+' - '+name+(f' ({detail})' if detail and not ok else ''))
print(f'\nV37 real-run compatibility regression: {len(checks)-len(failed)}/{len(checks)} passed')
